...
type
  ...
  protected
    ...
    property BarEnabled: Boolean read FBarEnabled write SetBarEnabled;
...
uses
  PegtopColorDialogs, PegtopColorUtils, PegtopColorServices, Menus;
...
type
  ...
  protected
    procedure DoCommand(GradientBar: TPegtopCustomGradientBar; Command: Integer); override;
...
procedure TPegtopCustomGradientBar.SetInternalGradient(Value: TPegtopCustomColorGradient);
begin
...
var
  ...
  begin
    if Key.Enabled then begin
      ...
      // draw button:
      DrawButton(ACanvas, ButtonRect, State, (FFocusKey = Key) and Focused);
      // draw color:
      if Enabled and BarEnabled then begin
...
begin
  ...
  try
    ...
    PegtopDrawBounds32(Origin, Pitch, Rect(0, 0, TempBitmap.Width, TempBitmap.Height),
      Rect(0, 0, TempBitmap.Width, TempBitmap.Height), FLook, clBtnFace);
...
var
  I: Integer;
begin
  for I := 0 to FGradient.Color.KeyCount - 1 do begin
...
begin
  if Key <> NIL then begin
    ...
    FGradient.Color.RemoveKey(TPegtopColorKey(Key));
  end;
end;
...
begin
  Result := FGradient.Color.KeyCount;
end;
...
begin
  Result := FGradient.Color.IndexOfKey(TPegtopColorKey(Key));
end;
...
var
  I: Integer;
begin
  for I := 0 to FGradient.Opacity.KeyCount - 1 do begin
...
begin
  if Key <> NIL then begin
    ...
    FGradient.Opacity.RemoveKey(TPegtopOpacityKey(Key));
  end;
end;

function TPegtopOpacityGradientBar.GetCount: Integer;
begin
  Result := FGradient.Opacity.KeyCount;
end;
...
begin
  Result := FGradient.Opacity.IndexOfKey(TPegtopOpacityKey(Key));
end;
...
begin
  ...
  for I := Low(BalanceMenuCaptions) to High(BalanceMenuCaptions) do begin
    MenuItem := CreateItem(BalanceMenuCaptions[I], 200 + I);
...
procedure TPegtopColorGradientPopupMenu.DoCommand(GradientBar: TPegtopCustomGradientBar; Command: Integer);
const
  ...
  BalanceStrength: array[0..2] of Integer = (64, 128, 256);
var
...
begin
  ...
  case Command of
    5:        if ColorClipboard.HasColorGradient then begin
                ...
                try
                  if ColorClipboard.GetColorGradient(TempGradient) then
                    GradientBar.Gradient.Color.Assign(TempGradient.Color);
                finally
                  TempGradient.Free;
                end;
              end;
    8:        GradientBar.Gradient.Color.DistributeKeysEvenly;
    9:        GradientBar.Gradient.Color.FlipKeys;
    100..102: GradientBar.Gradient.Color.UnifyColorComponents(TPegtopColorKey(GradientBar.FFocusKey).Color, HSBColorComponents[Tag - 100]);
    104..106: GradientBar.Gradient.Color.UnifyColorComponents(TPegtopColorKey(GradientBar.FFocusKey).Color, RGBColorComponents[Tag - 104]);
    200..202: GradientBar.Gradient.Color.BalanceKeys(BalanceStrength[Command - 200]);
  end;
end;
...
begin
  ...
  if Sender is TMenuItem then begin
    Component := TPopupMenu(TMenuItem(Sender).GetParentMenu).PopupComponent;
    if Component is TPegtopCustomGradientBar then begin
      if (TComponent(Sender).Tag = 5) and ColorClipboard.HasColorGradient then begin
        Gradient := TPegtopColorGradient.Create([]);
        try
          if ColorClipboard.GetColorGradient(Gradient) then begin
            Gradient.Seamless := TPegtopCustomGradientBar(Component).Gradient.Seamless;
            Gradient.Draw(ACanvas,
...
procedure TPegtopOpacityGradientPopupMenu.DoCommand(GradientBar: TPegtopCustomGradientBar; Command: Integer);
const
  BalanceStrength: array[0..2] of Integer = (64, 128, 256);
var
  TempGradient: TPegtopCustomColorGradient;
begin
  ...
  case Command of
    5:        if ColorClipboard.HasColorGradient then begin
                ...
                try
                  if ColorClipboard.GetColorGradient(TempGradient) then
                    GradientBar.Gradient.Opacity.Assign(TempGradient.Opacity);
                finally
                  TempGradient.Free;
                end;
              end;
    8:        GradientBar.Gradient.Opacity.DistributeKeysEvenly;
    9:        GradientBar.Gradient.Opacity.FlipKeys;
    200..202: GradientBar.Gradient.Opacity.BalanceKeys(BalanceStrength[Command - 200]);
  end;
end;
    ...
    if Component is TPegtopCustomGradientBar then begin
      if (TComponent(Sender).Tag = 5) and ColorClipboard.HasColorGradient then begin
        ...
        try
          if ColorClipboard.GetColorGradient(Gradient) then begin
            Gradient.Seamless := TPegtopCustomGradientBar(Component).Gradient.Seamless;
            Gradient.GrayscaleOpacity(clWhite, clBlack, ACanvas,
              Rect(ARect.Left + 3, ARect.Top + 3, ARect.Left + ARect.Bottom - ARect.Top - 3, ARect.Bottom - 3),
              Point(ARect.Left + 3, ARect.Top + 3), Point(ARect.Left + ARect.Bottom - ARect.Top - 3, ARect.Bottom - 3));
            ACanvas.Brush.Style := bsClear;
            ACanvas.Pen.Color := clBlack;
