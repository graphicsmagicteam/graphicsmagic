...
uses
  ...
  Controls,
  GR32,
  ColorTransformDlg in 'ColorTransformDlg.pas' {frmColorTransform},
...
begin
  ...
  frmColorTransform  := TfrmColorTransform.Create(Application);
  try
    try
      frmColorTransform.FDestBmpPtr     := DestBmpPtr;
      frmColorTransform.FUpdateViewProc := UpdateViewProc;
      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmColorTransform.FSourceBmp);

      if frmColorTransform.ShowModal = mrOK then
      begin
        if not frmColorTransform.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmColorTransform.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmColorTransform.FSourceBmp, DestBmpPtr, Width, Height);
