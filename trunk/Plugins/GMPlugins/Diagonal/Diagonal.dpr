...
uses
  ...
  Controls,
  GR32,
  DiagonalDlg in 'DiagonalDlg.pas' {frmDiagonal},
...
begin
  ...
  frmDiagonal        := TfrmDiagonal.Create(Application);
  try
    try
      frmDiagonal.FDestBmpPtr     := DestBmpPtr;
      frmDiagonal.FUpdateViewProc := UpdateViewProc;
      frmDiagonal.FBKColor        := BKColor;
      frmDiagonal.FChannelSet     := SelectedChannelSet;
      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmDiagonal.FSourceBmp);

      if frmDiagonal.ShowModal = mrOK then
      begin
        if not frmDiagonal.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmDiagonal.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmDiagonal.FSourceBmp, DestBmpPtr, Width, Height);
