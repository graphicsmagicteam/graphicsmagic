...
uses
  ...
  Controls,
  GR32,
  EmbossDlg in 'EmbossDlg.pas' {frmEmboss},
  AboutDlg in 'AboutDlg.pas' {frmAboutEmboss},
...
begin
  ...
  frmEmboss          := TfrmEmboss.Create(Application);
  try
    try
      frmEmboss.FDestBmpPtr     := DestBmpPtr;
      frmEmboss.FUpdateViewProc := UpdateViewProc;
      frmEmboss.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmEmboss.FSourceBmp);

      if frmEmboss.ShowModal = mrOK then
      begin
        if not frmEmboss.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmEmboss.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmEmboss.FSourceBmp, DestBmpPtr, Width, Height);
