...
uses
  ...
  Controls,
  GR32,
  ExpBlurDlg in 'ExpBlurDlg.pas' {frmExpBlur},
  AboutExpBlurDlg in 'AboutExpBlurDlg.pas' {frmAboutExpBlur},
...
begin
  ...
  frmExpBlur         := TfrmExpBlur.Create(Application);
  try
    try
      frmExpBlur.FDestBmpPtr     := DestBmpPtr;
      frmExpBlur.FUpdateViewProc := UpdateViewProc;
      frmExpBlur.FChannelSet     := SelectedChannelSet;
      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmExpBlur.FSourceBmp);

      if frmExpBlur.ShowModal = mrOK then
      begin
        if not frmExpBlur.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmExpBlur.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmExpBlur.FSourceBmp, DestBmpPtr, Width, Height);
