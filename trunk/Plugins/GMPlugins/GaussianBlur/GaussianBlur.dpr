...
uses
  ...
  Controls,
  GR32,
  GaussianBlurDlg in 'GaussianBlurDlg.pas' {frmGaussianBlur},
...
begin
  ...
  frmGaussianBlur    := TfrmGaussianBlur.Create(Application);
  try
    try
      frmGaussianBlur.FDestBmpPtr     := DestBmpPtr;
      frmGaussianBlur.FUpdateViewProc := UpdateViewProc;
      frmGaussianBlur.FChannelSet     := SelectedChannelSet;
      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmGaussianBlur.FSourceBmp);

      if frmGaussianBlur.ShowModal = mrOK then
      begin
        if not frmGaussianBlur.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmGaussianBlur.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmGaussianBlur.FSourceBmp, DestBmpPtr, Width, Height);
