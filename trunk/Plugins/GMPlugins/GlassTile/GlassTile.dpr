...
uses
  ...
  Controls,
  GR32,
  GlassTileDlg in 'GlassTileDlg.pas' {frmGlassTile},
  AboutGlassTileDlg in 'AboutGlassTileDlg.pas' {frmAboutGlassTile},
...
begin
  ...
  frmGlassTile       := TfrmGlassTile.Create(Application);
  try
    try
      frmGlassTile.FDestBmpPtr     := DestBmpPtr;
      frmGlassTile.FUpdateViewProc := UpdateViewProc;
      frmGlassTile.FChannelSet     := SelectedChannelSet;
      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmGlassTile.FSourceBmp);

      if frmGlassTile.ShowModal = mrOK then
      begin
        if not frmGlassTile.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmGlassTile.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmGlassTile.FSourceBmp, DestBmpPtr, Width, Height);
