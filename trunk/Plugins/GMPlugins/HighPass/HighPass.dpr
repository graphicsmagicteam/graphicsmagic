...
uses
  ...
  Controls,
  GR32,
  HighPassDlg in 'HighPassDlg.pas' {frmHighPass},
...
begin
  ...
  frmHighPass        := TfrmHighPass.Create(Application);
  try
    try
      frmHighPass.FDestBmpPtr     := DestBmpPtr;
      frmHighPass.FUpdateViewProc := UpdateViewProc;
      frmHighPass.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmHighPass.FSourceBmp);

      if frmHighPass.ShowModal = mrOK then
      begin
        if not frmHighPass.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmHighPass.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmHighPass.FSourceBmp, DestBmpPtr, Width, Height);
