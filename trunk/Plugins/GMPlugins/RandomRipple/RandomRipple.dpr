...
uses
  ...
  Controls,
  GR32,
  RandomRippleDlg in 'RandomRippleDlg.pas' {frmRandomRipple},
...
begin
  ...
  frmRandomRipple    := TfrmRandomRipple.Create(Application);
  try
    try
      frmRandomRipple.FDestBmpPtr     := DestBmpPtr;
      frmRandomRipple.FUpdateViewProc := UpdateViewProc;
      frmRandomRipple.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmRandomRipple.FSourceBmp);

      if frmRandomRipple.ShowModal = mrOK then
      begin
        if not frmRandomRipple.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmRandomRipple.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmRandomRipple.FSourceBmp, DestBmpPtr, Width, Height);
