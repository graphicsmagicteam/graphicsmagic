...
uses
  ...
  Controls,
  GR32,
  SolarizeDlg in 'SolarizeDlg.pas' {frmSolarize},
...
begin
  ...
  frmSolarize        := TfrmSolarize.Create(Application);
  try
    try
      frmSolarize.FDestBmpPtr     := DestBmpPtr;
      frmSolarize.FUpdateViewProc := UpdateViewProc;
      frmSolarize.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmSolarize.FSourceBmp);

      if frmSolarize.ShowModal = mrOK then
      begin
        if not frmSolarize.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmSolarize.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmSolarize.FSourceBmp, DestBmpPtr, Width, Height);
