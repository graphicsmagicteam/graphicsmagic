...
uses
  ...
  Controls,
  GR32,
  SplitColorDlg in 'SplitColorDlg.pas' {frmSplitColor},
...
begin
  ...
  frmSplitColor      := TfrmSplitColor.Create(Application);
  try
    try
      frmSplitColor.FDestBmpPtr     := DestBmpPtr;
      frmSplitColor.FUpdateViewProc := UpdateViewProc;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmSplitColor.FSourceBmp);

      if frmSplitColor.ShowModal = mrOK then
      begin
        if not frmSplitColor.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmSplitColor.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmSplitColor.FSourceBmp, DestBmpPtr, Width, Height);
