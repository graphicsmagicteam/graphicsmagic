...
uses
  ...
  Controls,
  GR32,
  SqueezeDlg in 'SqueezeDlg.pas' {frmSqueeze},
...
begin
  ...
  frmSqueeze         := TfrmSqueeze.Create(Application);
  try
    try
      frmSqueeze.FDestBmpPtr     := DestBmpPtr;
      frmSqueeze.FUpdateViewProc := UpdateViewProc;
      frmSqueeze.FBKColor        := BKColor;
      frmSqueeze.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmSqueeze.FSourceBmp);

      if frmSqueeze.ShowModal = mrOK then
      begin
        if not frmSqueeze.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmSqueeze.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmSqueeze.FSourceBmp, DestBmpPtr, Width, Height);
