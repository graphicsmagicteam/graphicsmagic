...
uses
  ...
  Controls,
  GR32,
  TwistDlg in 'TwistDlg.pas' {frmTwist},
...
begin
  ...
  frmTwist           := TfrmTwist.Create(Application);
  try
    try
      frmTwist.FDestBmpPtr     := DestBmpPtr;
      frmTwist.FUpdateViewProc := UpdateViewProc;
      frmTwist.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmTwist.FSourceBmp);

      if frmTwist.ShowModal = mrOK then
      begin
        if not frmTwist.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmTwist.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmTwist.FSourceBmp, DestBmpPtr, Width, Height);
