...
uses
  ...
  Controls,
  GR32,
  UnsharpMaskDlg in 'UnsharpMaskDlg.pas' {frmUnsharpMask},
...
begin
  ...
  frmUnsharpMask     := TfrmUnsharpMask.Create(Application);
  try
    try
      frmUnsharpMask.FDestBmpPtr     := DestBmpPtr;
      frmUnsharpMask.FUpdateViewProc := UpdateViewProc;
      frmUnsharpMask.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmUnsharpMask.FSourceBitmap);

      if frmUnsharpMask.ShowModal = mrOK then
      begin
        if not frmUnsharpMask.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmUnsharpMask.FProcessedBitmap, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmUnsharpMask.FSourceBitmap, DestBmpPtr, Width, Height);
