...
uses
  ...
  Controls,
  GR32,
  WaveDlg in 'WaveDlg.pas' {frmWave},
...
begin
  ...
  frmWave            := TfrmWave.Create(Application);
  try
    try
      frmWave.FDestBmpPtr     := DestBmpPtr;
      frmWave.FUpdateViewProc := UpdateViewProc;
      frmWave.FBKColor        := BKColor;
      frmWave.FChannelSet     := SelectedChannelSet;

      CopyBmpDataFromPtr(DestBmpPtr, Width, Height, frmWave.FSourceBmp);

      if frmWave.ShowModal = mrOK then
      begin
        if not frmWave.chckbxPreview.Checked then
        begin
          CopyBmpDataToPtr(frmWave.FProcessedBmp, DestBmpPtr, Width, Height);
      ...
      begin
        CopyBmpDataToPtr(frmWave.FSourceBmp, DestBmpPtr, Width, Height);
